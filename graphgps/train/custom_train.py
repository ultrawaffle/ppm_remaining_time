...
def custom_train(loggers, loaders, model, optimizer, scheduler):
    ...
    for cur_epoch in range(start_epoch, cfg.optim.max_epoch):
        ...
        if is_eval_epoch(cur_epoch):
            ...
            # Best checkpoint based on mae
            #if cfg.train.enable_ckpt and cfg.train.ckpt_best and \
            #        best_epoch == cur_epoch:
            #    save_ckpt(model, optimizer, scheduler, 'mae_ckpt')
            ...
            if cfg.train.enable_ckpt and cfg.train.ckpt_best and \
                    best_epoch == cur_epoch:
                path = os.path.join(cfg.run_dir, 'ckpt')
                ckpt: Dict[str, Any] = {}
                ...
                torch.save(ckpt, path + '/ckpt_best_mae.ckpt')

            # Custom checkpoint here based on nmae
            """
            current_nmae = perf[1][-1]['nmae']
            if current_nmae < best_val_nmae and cfg.train.enable_ckpt and cfg.train.ckpt_best:
                best_val_nmae = current_nmae
                path = os.path.join(cfg.run_dir, 'ckpt')
                ckpt: Dict[str, Any] = {}
                ckpt['model_state'] = model.state_dict()
                if optimizer is not None:
                ...
                os.makedirs(path, exist_ok=True)
                torch.save(ckpt, path + '/ckpt_best_nmae.ckpt')
            """
            ...
            if best_epoch == cur_epoch:
                ...
                if current_patience >= early_stop_patience:
                    print('Early stopping: Val loss has not improved for {} epochs.'.format(early_stop_patience))
                    break
