...
def train_model(model, 
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, pin_memory=True, generator=torch.Generator(device))
    ...
    for epoch in range(start_epoch, start_epoch + num_epochs):
        ...
        print(f'num_epochs_not_improved: {num_epochs_not_improved}')
        #if num_epochs_not_improved == 59:
        if num_epochs_not_improved == 5:
            print("No improvements in validation loss for 20 consecutive epochs. Final epoch: {}".format(epoch))
            break
