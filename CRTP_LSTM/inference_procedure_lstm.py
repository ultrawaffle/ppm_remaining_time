...
def inference_loop(model, 
    ...
    with torch.no_grad():
        ...
        suffix_acts_decoded_global = torch.empty((0, window_size), dtype=torch.int64).to(device)
        suffix_rrt_preds_global = torch.empty((0, window_size), dtype=torch.float32).to(device)
        ...
        rrt_label_fin_seconds_ = torch.tensor(data=[], dtype=torch.float32).to(device)
        rrt_preds_fin_seconds_ = torch.tensor(data=[], dtype=torch.float32).to(device)
        ...
        for valbatch_num, vdata in tqdm(enumerate(inference_dataloader), desc="Validation batch calculation"):
                ...
                suffix_acts_decoded = infer_env.suffix_acts_decoded.clone().to(device) # (batch_size, window_size)
                suffix_acts_decoded_global = torch.cat((suffix_acts_decoded_global, suffix_acts_decoded), dim=0) 

                suffix_rrt_preds = infer_env.rrt_preds.clone().to(device) # (batch_size, window_size)
                suffix_rrt_preds_global = torch.cat((suffix_rrt_preds_global, suffix_rrt_preds), dim=0)
                ...
                # Evaluation RRT metrics: only the first rrt prediction is to be used upon 
                # evaluation
                MAE_rrt_stand, MAE_rrt_seconds, label, pred = infer_env.compute_rrt_results()
        ...
        if results_path: 
            ...
            #Predictions
            pa = os.path.join(subfolder_path, 'pred.pt')
            torch.save(rrt_preds_fin_seconds_, pa)
            #Predictions - label
            pa = os.path.join(subfolder_path, 'pred-labels.pt')
            torch.save(rrt_label_fin_seconds_, pa)
