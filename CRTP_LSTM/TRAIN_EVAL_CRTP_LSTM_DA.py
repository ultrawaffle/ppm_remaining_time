...
def train_eval(log_name, input_data_location, results_path):
    ...
    log_name_1 = log_name
    log_name = input_data_location

    # -----------------
    temp_string = log_name_1 + '_cardin_dict.pkl'
    ...
    temp_string = log_name_1 + '_cardin_list_prefix.pkl'
    temp_path = os.path.join(log_name, temp_string)
    cardinality_list_prefix = load_dict(temp_path)

    temp_string = log_name_1 + '_cardin_list_suffix.pkl'
    temp_path = os.path.join(log_name, temp_string)
    # cardinality list suffix categoricals
    cardinality_list_suffix = load_dict(temp_path)

    temp_string = log_name_1 + '_num_cols_dict.pkl'
    temp_path = os.path.join(log_name, temp_string)
    ...
    temp_string = log_name_1 + '_cat_cols_dict.pkl'
    temp_path = os.path.join(log_name, temp_string)
    cat_cols_dict = load_dict(temp_path)

    temp_string = log_name_1 + '_train_means_dict.pkl'
    temp_path = os.path.join(log_name, temp_string)
    train_means_dict = load_dict(temp_path)

    temp_string = log_name_1 + '_train_std_dict.pkl'
    temp_path = os.path.join(log_name, temp_string)
    ...
    d_model = 200 #40 #50 # divided by 2 = hidden size #80-default
    dropout = 0.2
    batch_size = 128 #32 #64<- next time #128
    ...
    backup_path = os.path.join(results_path, "CRTP_LSTM_DA_results")
    ...
    print(f'best epoch: {epoch_value}')
...
#    train_dataset = train_dataset.to(device)
    ...
    d_model = 32 #50 # divided by 2 = hidden size #80-default
    dropout = 0.5 #0.2
...
if __name__ == '__main__':
    print('Start train_eval_sutran_da.py')
    # if sys.argv[2] == 'eval':
    #     eval(sys.argv[1])
    # else:
    train_eval(sys.argv[1])
