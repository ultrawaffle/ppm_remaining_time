...
def inference_loop(model, 
    ...
    * `num_activities` : the total number of possible activity labels 
    ...
    inference_dataloader = DataLoader(inf_tensordataset, batch_size=val_batch_size, shuffle=False, drop_last=False, pin_memory=True, generator=torch.Generator(device))
    ...
    with torch.no_grad():
        ...
        if remaining_runtime_head:
            ...
            rrt_pred_global = torch.tensor(data=[], dtype=torch.float32).to(device)
        
        if outcome_bool:
            # Shape (number of instances, ) after inference loop
            out_pred_global = torch.tensor(data=[], dtype=torch.float32).to(device)
        ...
        pref_len_global = torch.where(pref_len_global == 0, window_size, pref_len_global).to(device) # (num_prefs,)
        ...
        for valbatch_num, vdata in tqdm(enumerate(inference_dataloader), desc="Validation batch calculation"):
                ...
                #suffix_acts_decoded_global = torch.cat((suffix_acts_decoded_global, suffix_acts_decoded.cpu()), dim=0).to(device)
                suffix_acts_decoded_global = torch.cat((suffix_acts_decoded_global, suffix_acts_decoded), dim=0).to(device)
                ...
                suffix_ttne_preds_global = torch.cat((suffix_ttne_preds_global, suffix_ttne_preds), dim=0).to(device)
                ...
                if only_rrt: 
                    ...
                    rrt_pred_global = torch.cat((rrt_pred_global, rrt_pred), dim=-1)

                elif only_out:
                    # - (binary) outcome prediction
                    out_pred = voutputs[-1] # (B, ) torch.float32
                    #out_pred_global = torch.cat((out_pred_global, out_pred.cpu()), dim=-1)
                    out_pred_global = torch.cat((out_pred_global, out_pred), dim=-1)
        ...
        outputs_global = (suffix_acts_decoded_global.to(device), suffix_ttne_preds_global.to(device))
        ...
        if remaining_runtime_head:
            MAE_rrt_stand, MAE_rrt_seconds, label, pred = infer_env.compute_rrt_results() # (num_prefs,)

            #rrt_label_fin_seconds_ = torch.cat(tensors=(rrt_label_fin_seconds_, label), dim=-1)
            #rrt_preds_fin_seconds_= torch.cat(tensors=(rrt_preds_fin_seconds_, pred), dim=-1)
        ...
        counting_tensor = counting_tensor.to(device)
        suf_len_global = suf_len_global.to(device)
        before_end_token = counting_tensor <= (suf_len_global-1).unsqueeze(-1) # (num_prefs,)
        
        MAE_ttne_stand = MAE_ttne_stand.to(device)
        ...
        MAE_ttne_seconds = MAE_ttne_seconds.to(device)
        ...
        if results_path:
            ...
            if remaining_runtime_head:
                ...
                labelpath = os.path.join(subfolder_path, 'pred-label.pt')
                torch.save(label, labelpath)

                predpath = os.path.join(subfolder_path, 'pred.pt')
                torch.save(pred, predpath)
