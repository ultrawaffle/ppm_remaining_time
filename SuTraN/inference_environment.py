...
class BatchInference():
    def __init__(self,
        ...
        * `num_activities` : the total number of possible activity labels 
        ...
        self.rrt_preds = self.rrt_preds.to(device)
    ...
    def edit_suffix_acts(self):
        ...
        pred_length = pred_length.to(device)
    ...
    def convert_to_seconds(self, time_string, input_tensor):
        ...
        converted_tensor = converted_tensor.to(device)
    ...
    def compute_ttne_results(self):
        ...
        self.suffix_ttne_preds = self.suffix_ttne_preds.to(device)
        self.ttne_labels = self.ttne_labels.to(device)
    ...
    def compute_rrt_results(self):
        ...
        rrt_preds_seconds = self.convert_to_seconds(time_string='rrt', input_tensor=self.rrt_preds.clone()).to(device) # (num_prefs,)
        self.rrt_preds = self.rrt_preds.to(device)
        self.rrt_labels = self.rrt_labels.to(device)
        self.rrt_labels_seconds = self.rrt_labels_seconds.to(device)
        ...
        return abs_errors, abs_errors_seconds, self.rrt_labels_seconds, rrt_preds_seconds # both (batch_size,)
    ...
    def damerau_levenshtein_distance_tensors(self):
        ...
        * `self.actual_length` : torch.Tensor of dtype torch.int64 and 
        ...
        len_pred = len_pred.to(device)
        len_actual = len_actual.to(device)
        ...
        self.suffix_acts_decoded = self.suffix_acts_decoded.to(device)
        self.act_labels = self.act_labels.to(device)
